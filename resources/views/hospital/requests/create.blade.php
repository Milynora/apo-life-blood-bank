<x-app-layout title="Submit Blood Request">
  ...
  <div class="page-header">
    <div>
      ...
      <h1 class="page-title">Submit Blood Request</h1>
      <p class="page-subtitle">Request blood units from Apo Life. Reviewed within 24–48 hours.</p>
    </div>
  ...
  <form method="POST" action="{{ route('hospital.requests.store') }}"
    x-data="{ qty: {{ old('quantity', 1) }}, fulfillment: '{{ old('fulfillment', 'pickup') }}' }">
    @csrf
              ...
              <div class="form-group" style="margin-bottom:0;">
                ...
                <select name="blood_type_id" required class="form-input form-input-light">
                  ...
                  @foreach($bloodTypes as $bt)
                    <option value="{{ $bt->blood_type_id }}" {{ old('blood_type_id') == $bt->blood_type_id ? 'selected' : '' }}>
                      {{ $bt->type_name }}
                    </option>
                  @endforeach
                </select>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1.25rem;">
              <div class="form-group" style="margin-bottom:0;">
                ...
                <select name="urgency" required class="form-input form-input-light">
                  <option value="routine"   {{ old('urgency','routine') === 'routine'   ? 'selected' : '' }}>Routine (24–48 hrs)</option>
                  <option value="urgent"    {{ old('urgency','routine') === 'urgent'    ? 'selected' : '' }}>Urgent (12–24 hrs)</option>
                  <option value="emergency" {{ old('urgency','routine') === 'emergency' ? 'selected' : '' }}>Emergency (Immediately)</option>
                </select>
              ...
              <div class="form-group" style="margin-bottom:0;">
                ...
                <select name="fulfillment" x-model="fulfillment" required class="form-input form-input-light">
                  <option value="pickup"   {{ old('fulfillment','pickup') === 'pickup'   ? 'selected' : '' }}>Pickup at blood bank</option>
                  <option value="delivery" {{ old('fulfillment','pickup') === 'delivery' ? 'selected' : '' }}>Delivery to hospital</option>
                </select>
    ...
    @error('needed_by')<div class="form-error">{{ $message }}</div>@enderror
            ...
            <div class="form-group" style="margin-bottom:1.25rem;">
              ...
              <textarea name="remarks" rows="3"
                class="form-input form-input-light"
                style="resize:none;"
                placeholder="e.g. For surgery scheduled tomorrow. Patient is adult male.">{{ old('remarks') }}</textarea>
            ...
            <div style="background:rgba(41,128,185,0.06); border:1px solid rgba(41,128,185,0.18); border-radius:10px; padding:0.75rem 1rem; display:flex; gap:0.6rem; align-items:flex-start; margin-bottom:1.25rem;">
              ...
              <p style="font-size:0.78rem; color:#1a5276; line-height:1.6; margin:0;">
                Your request will be reviewed within <strong>24–48 hours</strong>.
              </p>
            </div>
        ...
        <div class="dash-card">
          ...
          <div class="dash-card-body" style="padding:1rem 1.25rem;">
            ...
            <div style="font-size:0.75rem; color:#888; line-height:1.5;">
              <br>Check availability before submitting. Low or 0 units may delay fulfillment.
            </div>
          </div>
        </div>
