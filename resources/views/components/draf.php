<nav class="guest-nav" id="guest-nav">
  ...
  <a href="{{ route('home') }}" class="nav-logo">
    <img src="{{ asset('images/logo.jpg') }}" alt="Apo Life Logo"/>
  ...
  <div style="display:flex; align-items:center; gap:0.6rem;">
    @guest
      ...
      <a href="{{ route('register') }}" class="btn btn-nav-register">Register</a>
    @endguest
    @auth
      @php
      ...
      @endphp
      <a href="{{ route($dashRoute) }}" class="btn btn-nav-register">Dashboard</a>
    @endauth
...
<div id="guest-mobile-menu"
  ...
  <ul style="list-style:none; display:flex; flex-direction:column; gap:0.25rem; margin-bottom:1.25rem;">
    @foreach([
      ...
      <li>
        <a href="{{ route('home') }}{{ $href }}"
          style="display:block; color:rgba(240,240,245,0.85); text-decoration:none; padding:0.7rem 1rem; border-radius:8px; font-size:0.95rem; transition:background 0.2s;"
  ...
  @guest
    <div style="display:flex; gap:0.75rem; flex-wrap:wrap;">
      <a href="{{ route('login') }}"    class="btn btn-outline" style="flex:1; text-align:center;">Login</a>
      <a href="{{ route('register') }}" class="btn btn-primary"  style="flex:1; text-align:center;">Register</a>
    </div>
  @endguest
</div>
...
.guest-nav-link:hover::after { transform: translateX(-50%) scaleX(0.6); }
